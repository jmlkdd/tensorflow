...
def conv2d(name, img, w, b):
    return tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(img, w, strides=[1, 1, 1, 1], name=name, padding='SAME'), b))


def max_pool(name, img, k):
    return tf.nn.max_pool(img, ksize=[1, k, k, 1], strides=[1, k, k, 1], name=name, padding='SAME')
