...
# Network Parameters
n_input = 784   # data_sets input (img shape: 28*28)
n_classes = 10  # data_sets total classes (0-9 digists)
dropout = 0.75
...
def conv_net(_X, _weights, _biases, _dropout):
    # Reshape imput picture
    _X = tf.reshape(_X, shape=[-1, 28, 28, 1])

    # Convolution Layer1
    conv1 = conv2d(_X, _weights['wc1'], _biases['bc1'])     # Convolution Layer
    conv1 = max_pool(conv1, k=2)        # Max Pooling
    conv1 = tf.nn.dropout(conv1, _dropout)      # Apply Dropout

    # Convolution Layer2
    conv2 = conv2d(conv1, _weights['wc2'], _biases['bc2'])
    conv2 = max_pool(conv2, k=2)  # Max Pooling
    conv2 = tf.nn.dropout(conv2, _dropout)  # Apply Dropout
...
with tf.Session() as sess:
    ...
    print "Testing Accuracy:", sess.run(accuracy, feed_dict={x: data_sets.test.images[:256], y: data_sets.test.labels[:256], keep_prob: 1.})
